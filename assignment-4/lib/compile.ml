...
(** [operand_of_nil ()] returns the runtime representation of the nil value as
    an operand for instructions *)
let operand_of_nil : unit -> operand =
  fun _ -> 
    Imm (0 lor nil_tag)
...
let ensure_positive_num : operand -> s_exp -> directive list = 
  fun op e ->
    ensure_num op e @ 
    let msg_label = gensym "emesg" in
    let continue_label = gensym "continue" in 
      [ Cmp (op, operand_of_num 0)
      ; Jg continue_label
      ; LeaLabel (Reg Rdi, msg_label)
      ; Jmp "lisp_error"
      ; Label msg_label
...
let ensure_non_negative : operand -> s_exp -> directive list = 
  fun op e ->
    ensure_num op e @ 
    let msg_label = gensym "emesg" in
    let continue_label = gensym "continue" in 
...
let ensure_vaild_index : operand -> operand -> s_exp -> directive list = 
  fun index len e -> 
    ensure_non_negative (index) e 
    @ let msg_label = gensym "emsg" in 
      let continue_label = gensym "continue" in
      [ Cmp (index, len)
      ; Jl continue_label
...
let compile_unary_primitive : s_exp -> string -> directive list =
  fun e prim ->
    begin match prim with
      ...
      | "list?" ->
          is_pair
          @ let value_label = gensym "value" in
          [Jne value_label]
          @ let right_label = gensym "right" in
          [Label right_label]
          @ [ Mov (Reg Rax, MemOffset (Reg Rax, Imm (-pair_tag + 8)))]
          @ is_pair 
          @ [Je right_label]
...
let compile_binary_primitive : int -> s_exp -> string -> directive list =
  fun stack_index e prim ->
    begin match prim with
      ...
      | "vector" ->
          let store_value_label = gensym "store_value" in
          ensure_positive_num (stack_address stack_index) e
            @ [ Mov (Reg Rcx, stack_address stack_index)
              ; Mov (MemOffset (Reg Rdi, Imm 0), Reg Rcx)
              ; Shr (Reg Rcx, Imm 2)
              ; Mov (Reg R8, Imm 8)
              ; Label store_value_label
              ; Mov (MemOffset (Reg Rdi, Reg R8), Reg Rax)
              ; Add (Reg R8, Imm 8)
              ; Loop store_value_label
              ; Mov (Reg Rax, Reg Rdi)
      ...
      | "vector-get" ->
          ...
          @ [ Mov (Reg R9, stack_address stack_index)
            ; Mov (Reg R9, MemOffset(Reg R9, Imm (-vector_tag)))
            ]
          @ ensure_vaild_index (Reg Rax) (Reg R9) e 
          @ [ Mov (Reg R8, stack_address stack_index)
            ; Sub (Reg R8, Imm vector_tag)
            ; Shr (Reg Rax, Imm 2)
            ; Add (Reg Rax, Imm 1)
            ; Mov (Reg R9, Imm 8)
            ; Mul (Reg R9)
            ; Mov (Reg Rax, MemOffset(Reg R8, Reg Rax))
            ]
  ...
  fun stack_index e prim ->
    begin match prim with
      | "vector-set" ->
          ...
          @ ensure_vaild_index (stack_address (stack_index - 8)) (Reg R9) e
          @ [ Mov (Reg R8, stack_address stack_index)
            ; Sub (Reg R8, Imm vector_tag)
            ...
            ; Mul (Reg R9)
            ; Mov (Reg R9, stack_address (stack_index - 8))
            ; Mov (MemOffset(Reg R8, Reg Rax), Reg R9)
            ; Mov (Reg Rax, stack_address stack_index)
            ]
      | _ ->
          raise (Error.Stuck e)
