...
let rec display_value : value -> string =
  fun v ->
    begin match v with
      ...
      | Vec v -> "[" ^ String.concat " " (Array.to_list (Array.map display_value v)) ^ "]"
      | Nil -> "()"    
    end
...
let interp_unary_primitive : string -> value -> value option =
  fun prim arg ->
    begin match (prim, arg) with
      ...
      | ("list?", v) ->
          let rec ends_with_nil = 
            fun v ->
                match v with 
                | Nil -> true 
                | Pair (_, v2) -> ends_with_nil v2
                | _ -> false
          in Some (Bool (ends_with_nil v))
...
let interp_binary_primitive : string -> value -> value -> value option =
  fun prim arg1 arg2 ->
    begin match (prim, arg1, arg2) with
      ...
      | ("vector", Num n, v) when n > 0 ->
          Some (Vec (Array.make n v)) 

      | ("vector-get", Vec v, Num n) when n >= 0 && n < Array.length v -> 
          Some (v.(n))    
      | _ ->
          None
    end
 ...
 string -> value -> value -> value -> value option =
  fun prim arg1 arg2 arg3 ->
    begin match (prim, arg1, arg2, arg3) with
      | ("vector-set", Vec v, Num n, e) when n >= 0 && n < Array.length v -> 
          v.(n) <- e;
          Some (Vec v)
      | _ ->
        None
