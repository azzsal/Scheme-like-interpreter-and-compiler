...
let rec interp_expr : environment -> s_exp -> value =
  fun env e ->
    begin match e with
      ...
      | Lst [Sym "let"; Lst bindings_exps_lst; body] -> 
          let bindings = 
            get_bindings bindings_exps_lst 
          in 
          let env = 
            List.fold_left (fun acc (name, s_exp) -> 
              Symtab.add name (interp_expr env s_exp) acc) env bindings 
          in   
          interp_expr env body
      ...
      | Lst [Sym "or"; arg1; arg2] ->
          let arg1_value = interp_expr env arg1 in
          if arg1_value <> Bool false then
            arg1_value 
          else 
            interp_expr env arg2

      | Lst (Sym "case" :: scrutinee :: cases) ->
          begin match interp_expr env scrutinee with
            | Num n -> 
                let case_list = get_cases cases in
                begin match List.assoc_opt n (get_cases cases) with 
                  | Some s_exp -> interp_expr env s_exp
                  | None -> interp_expr env 
                  (get_second (List.nth case_list (List.length case_list - 1))) 
                end
