  ...
  fun tab stack_index e ->
    begin match e with
      ...
      | Lst [Sym "let"; Lst bindings_exps_lst; body] -> 
          let bindings = List.mapi (fun i (name, s_exp) ->
            (i, name, s_exp)) (get_bindings bindings_exps_lst) in
          List.concat_map (fun (i, _, s_exp) -> 
            compile_expr tab (stack_index - (i * 8)) s_exp
            @ [Mov (stack_address (stack_index - (i * 8)), Reg Rax)]) 
            bindings
          @ compile_expr 
              (List.fold_left (fun acc (i, name, _) -> 
                Symtab.add name (stack_index - (i * 8)) acc) tab bindings)
