...
let rec interp_exp (defns: defn list) (env: value symtab) (exp: s_exp) : value =
  match exp with
  | Lst (Sym name :: args) when is_defn defns name ->
  ...
  | Lst [Sym "pair"; e1; e2] ->
      ...
      Pair (l, r)
  | Lst [Sym "left"; e] -> begin
      match interp_exp defns env e with 
      | Pair (v, _) -> v 
      | _ -> raise (BadExpression exp) end
  | Lst [Sym "right"; e] -> begin 
      match interp_exp defns env e with 
      | Pair (_, v) -> v
      | _ -> raise (BadExpression exp) end
  ...
  | Lst [Sym "add1"; arg] -> begin
      match (interp_exp defns env arg) with
      | Number n -> Number (n + 1)
      | _ -> raise (BadExpression exp) end
  | Lst [Sym "sub1"; arg] -> begin 
      match (interp_exp defns env arg) with 
      | Number n -> Number (n - 1)
      | _ -> raise (BadExpression exp) end
  | Lst [Sym "not"; arg] -> 
...
let interp_io (program : string) (input : string) : string =
