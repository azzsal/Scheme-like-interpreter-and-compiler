...
(* `operand_of_char c` returns the runtime representation of the char `b` as
 * and operand for instrucations.
 *)
...
let compile_primitive : s_exp -> string -> directive list =
  fun e prim ->
    begin match prim with
      ...
      | "char->num" -> 
          [ Shr (Reg Rax, Imm (char_shift - num_shift))
          ; Or (Reg Rax, Imm num_tag)
          ]
          
      | "num->char" ->
          [ Shl (Reg Rax, Imm (char_shift - num_shift))
